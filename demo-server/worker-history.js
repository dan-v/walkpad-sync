// WalkPad Sync Demo API - HISTORY ONLY
// No data for today, only historical data (shows empty state on Today tab)
// Deploy: npx wrangler deploy --config wrangler-history.toml
...
function generateMockData(userToday, tzOffsetSeconds = 0) {
  ...
  for (let i = 1; i < 60; i++) { // Start at i=1 to SKIP today
    ...
    const baseSteps = isWeekend ? 2000 : 6000;
    const variance = isWeekend ? 3000 : 5000;
  ...
  async fetch(request, env, ctx) {
    ...
    if (path === '/api/health') {
      return new Response(JSON.stringify({
        status: 'ok',
        demo_mode: 'history',
        description: 'History only (no data for today)'
      }), { headers: corsHeaders });
    }
    ...
    if (path === '/api/bluetooth/status') {
      return new Response(JSON.stringify({
        connected: false,
        device_name: null,
        message: 'Demo mode - history only'
      }), { headers: corsHeaders });
    }
